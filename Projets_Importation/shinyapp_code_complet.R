...
library(jsonlite)   # Pour manipuler des objets JSON
library(shinyjs)    # Pour utiliser JavaScript dans Shiny
library(stringr)    # Pour manipuler des chaînes de caractères
library(dplyr)      # Pour la manipulation de données
library(readxl)     # Pour lire des fichiers Excel
library(curl)       # Pour gérer les connexions HTTP/FTP
library(readr)      # Pour lire des fichiers CSV
library(rsconnect)  # Pour déployer l'application sur un serveur

# Options pour ajuster les limites de taille des fichiers lors du déploiement
options(rsconnect.max.bundle.size = 300000000000)
options(rsconnect.max.bundle.files = 165000)

# Charger les données initiales depuis un fichier CSV en ligne
data_initial <- read.csv("https://www.dropbox.com/scl/fi/kmrtwqkbqof6ofa94mvvr/cordonn-_2.csv?rlkey=9j5uftpb4c6reu9o4ll3h2gtl&st=oic1udaa&dl=1", 
                         sep = ",", 
                         header = TRUE)    # Spécifier que la première ligne contient les noms des colonnes

# Ensuite, la définition de l'interface utilisateur et du serveur continue ici
...
server <- function(input, output, session) {
          ...
          mainPanel(
            tabsetPanel(
              ...
              tabPanel("Graphiques",
                       ...
                       downloadButton("download_histogram", "Télécharger Histogramme", class = "button-spacing"),
                       plotOutput("histogram"),
                       tags$br(),
                       
                       downloadButton("download_scatterplot", "Télécharger Nuage de points", class = "button-spacing"),
                       actionButton("show_correlation", "Afficher le coefficient de corrélation", class = "button-spacing"), 
                       plotOutput("scatterplot"), 
                       tags$br(),
                       
                       downloadButton("download_habitations_par_code_postal", "Télécharger Habitations par Code Postal", class = "button-spacing"),
                       plotOutput("habitations_par_code_postal"),
                       tags$img(src = "https://github.com/Utopie81/iut_sd2_rshiny_enedis_EMS2/raw/main/Projets_Importation/logo.webp",
                                style = "position: fixed; bottom: 10px; left: 10px; height: 250px;")
              ),
              tabPanel(
  ...
  output$prevention_text <- renderText({
    "Au vue du nombre de donées le refresh data peut prendre un certain temps"
  })
  ...
  output$histogram <- renderPlot({
    ...
    filtered_data_year$Year <- format(as.Date(filtered_data_year$Date_réception_DPE), "%Y")
    
    
    year_count <- as.data.frame(table(filtered_data_year$Year))
    colnames(year_count) <- c("Year", "Count")
    
    
    ggplot(year_count, aes(x = Year, y = Count)) + 
      geom_bar(stat = "identity", fill = "blue", color = "white") +
      geom_text(aes(label = Count), vjust = -0.5, color = "black", size = 4) +  # Ajouter les étiquettes
      labs(title = "Histogramme des DPE par Année", x = "Année de réception", y = "Nombre d'habitations") +
      theme_minimal() +
      scale_x_discrete(limits = sort(unique(year_count$Year)))  # Assurez-vous que les années sont affichées dans l'ordre
  })
  ...
  output$scatterplot <- renderPlot({
    ...
    ggplot(plot_data, aes(x = lon, y = lat, color = color)) + # Utilisez la colonne de couleur
      geom_point(size = 3, alpha = 0.7) + # Ajustez l'opacité si nécessaire
      geom_smooth(method = "lm", se = FALSE, color = "red", linetype = "dashed") +
      labs(title = "Nuage de points (Localisation) avec Régression", x = "Longitude", y = "Latitude") +
      theme_minimal() +
      scale_color_identity() # Utilisez cette ligne pour s'assurer que les couleurs sont prises directement de la colonne color
  })
  
  output$habitations_par_code_postal <- renderPlot({
    plot_data <- filtered_data()
    
    # Vérifier que la colonne 'Code_postal_(BAN)' existe
    if (!"Code_postal_(BAN)" %in% colnames(plot_data)) {
      stop("Erreur : La colonne 'Code_postal_(BAN)' est manquante.")
    }
    
    # Créer une nouvelle colonne pour les 2 premiers chiffres du code postal
    plot_data$Code_Postal_2_chiffres <- stringr::str_sub(plot_data$`Code_postal_(BAN)`, 1, 2)
    
    # Compter le nombre d'habitations par code postal (2 premiers chiffres)
    count_data <- plot_data %>%
      group_by(Code_Postal_2_chiffres) %>%
      summarise(Nombre_Habitations = n(), .groups = "drop")
    
    # Créer le graphique
    ggplot(count_data, aes(x = Code_Postal_2_chiffres, y = Nombre_Habitations, fill = Code_Postal_2_chiffres)) + 
      geom_bar(stat = "identity") +
      geom_text(aes(label = Nombre_Habitations), vjust = -0.5, color = "black") +
      labs(title = "Nombre d'Habitations par Code Postal (2 premiers chiffres)", 
           x = "Code Postal (2 premiers chiffres)", 
  ...
  observeEvent(input$show_correlation, {
    ...
    if (nrow(plot_data) > 1) {
      correlation <- cor(plot_data$lon, plot_data$lat, use = "complete.obs")
      showNotification(paste("Coefficient de corrélation:", round(correlation, 2)), type = "message")
    } else {
      showNotification("Pas assez de données pour calculer le coefficient de corrélation.", type = "error")
    }
  })
  ...
  output$download_habitations_par_code_postal <- downloadHandler(
    filename = function() {
      paste("habitations_par_code_postal", Sys.Date(), ".png", sep = "")
    },
  ...
  output$download_scatterplot <- downloadHandler(
    filename = function() {
      paste("nuage_points", Sys.Date(), ".png", sep = "")
    },
    content = function(file) {
      png(file)
      print(ggplot2::last_plot())
  ...
  output$download_histogram <- downloadHandler(
    filename = function() {
      paste("histogramme", Sys.Date(), ".png", sep = "")
    },
    content = function(file) {
      png(file)
      print(ggplot2::last_plot())
